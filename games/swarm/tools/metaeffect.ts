...
const cases:[string,Record<string,number>,number,number][]=[
 ['run 1   (no forge, no mastery, tier 0)',{},0,0],
 ['run 30+ (max forge, mastery 10, tier 0)',MAX,10,0],
 ['run 30+ (max forge, mastery 10, tier 8)',MAX,10,8],
];
for(const [name,f,m,a] of cases){
  const ts:number[]=[];
  for(let ci=0;ci<6;ci++)for(let s=0;s<6;s++) ts.push(run(3000+s*104729+ci*7919,ci,f,m,a));
  console.log(`${name.padEnd(42)} n=${ts.length} p25=${pc(ts,25).toFixed(0)}s p50=${pc(ts,50).toFixed(0)}s p75=${pc(ts,75).toFixed(0)}s p90=${pc(ts,90).toFixed(0)}s max=${Math.max(...ts).toFixed(0)}s`);
}
